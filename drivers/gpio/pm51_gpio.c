...
// min message size is 2 chars for single-byte ACK/NAK message
...
static int encodeCMD(unsigned char *inBuffer, int inLength,
		     unsigned char *outputBuffer, int *outputLength)
{
	unsigned char *ptr = outputBuffer;
	char checkSum = 0;
	int length = 2 * inLength +
		     4; // total length of output buffer if all goes well
	unsigned char val;
	int i;
	...
	*ptr++ = SYNC; // sync byte
	// length of message characters in buffer (excludes checksum chars)
	*ptr++ = 2 * inLength;
...
static ssize_t fan_io_proc_write(struct file *file, const char __user *buf,
