...
static ssize_t proc_board_io_write(struct file *file, const char __user *buf,
...
static void intrpt_routine(struct work_struct *unused)
{
	...
	pm51_read_gpio(PM_PWR_BTN,
		       &pwr_btn_state); //With Watchdog reset ever 50ms
	...
	if (module_die == 0)
		queue_delayed_work(my_workqueue, &btn_sched,
				   dyn_work_queue_timer);
	if (wdt_disable > 1)
		pm51_wdtset(
			1); //IEC-ADD by EDEN for wdt timeout reboot system at 20151113
}
				...
				size_t length, loff_t *ppos)
{
	...
	prepare_to_wait(&board_event_queue, &wait, TASK_INTERRUPTIBLE);
	schedule();
	finish_wait(&board_event_queue, &wait);
...
static void wdt_timeout(struct timer_list *x)
{
	struct wdt_setting *wdtset = from_timer(wdtset, x, timer);
	printk(KERN_ERR "wdt_timeout pid=%d, time_set=%d\n", wdtset->pid, wdtset->time_set);
	if (wdt_disable == 0)
		wdt_disable = 2;
}

static ssize_t proc_wdtsetting_write(struct file *file, const char __user *buf,
				     size_t length, loff_t *ppos)
{
...
static struct notifier_block sys_notifier_reboot = { .notifier_call =
							     sys_notify_reboot,
						     .next = NULL,
						     .priority = 0 };
...
#define PM_POST_SUSPEND 0x0004 /* Suspend finished */
	...
	if (event == PM_SUSPEND_PREPARE) {
		wdt_disable = 1;
		pm51_wdtset(
			0); //IEC-ADD by EDEN for kernel disable wdtSetWatchdog at 20151113
		printk(KERN_INFO "PM_SUSPEND_PREPARE : Disable WDT\n");
	} else if (event == PM_POST_SUSPEND) {
		printk(KERN_INFO "PM_POST_SUSPEND : Enable WDT\n");
		pm51_wdtset(
			100); //IEC-ADD by EDEN for kernel init wdtSetWatchdog 10 Sec at 20151113
		wdt_disable = 0;
	}
	return NOTIFY_DONE;
}

static struct notifier_block pm_notifier_board = { .notifier_call =
							   pm_notify_sleep,
						   .next = NULL,
						   .priority = 0 };
...
static __init int board_io_init(void)
{
	...
	//IEC-ADD by EDEN for kernel init wdtSetWatchdog 10 Sec at 20151113
	pm51_wdtset(100);
...
{
	//IEC-ADD by EDEN for kernel disable wdtSetWatchdog at 20151113
	pm51_wdtset(0);
...
MODULE_DESCRIPTION(
	" Inventec Barcelona MB Driver and board depend io operation");
MODULE_LICENSE("GPL");
