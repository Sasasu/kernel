...
struct tc358840_state {
	struct tc358840_dt_data dt_data;
...
static void tc358840_set_hdmi_hdcp(struct v4l2_subdev *sd, bool enable)
{
	...
	if (enable) {
		...
		if (state->dt_data.hdcp_repeater) {
			v4l2_info(sd,
...
{
	struct tc358840_state *state = to_state(sd);
	struct tc358840_dt_data *dt_data = &state->dt_data;
	u16 csi_port = dt_data->csi_port;
...
retry:
	...
	if (enable) {
		if (dt_data->endpoint.bus.mipi_csi2.flags &
...
static void tc358840_set_splitter(struct v4l2_subdev *sd)
{
	...
	/*
	 * Only enable the splitter if two CSI ports can be used
	 * and if the width > 1920.
	 * There are some odd timings with width <= 1920 that do not
	 * map very well to two CSI ports (e.g. 1366x768).
	 */
	if (state->dt_data.csi_port != CSI_TX_BOTH ||
	    state->timings.bt.width <= 1920) {
...
static int get_hsck_freq(struct tc358840_dt_data *dt_data)
{
	int hsck = (dt_data->refclk_hz / (dt_data->pll_prd + 1) *
		(dt_data->pll_fbd + 1)) / (1 << dt_data->pll_frs);
	return hsck;
}
			     ...
			     enum tc358840_csi_port port)
{
	...
	if (WARN_ON(dt_data->csi_port <= CSI_TX_NONE ||
		    dt_data->csi_port > CSI_TX_BOTH))
		dt_data->csi_port = CSI_TX_NONE;

	if (dt_data->csi_port == CSI_TX_NONE) {
	...
	base_addr = (port == CSI_TX_0) ? CSITX0_BASE_ADDR :
					CSITX1_BASE_ADDR;
	pllconf = SET_PLL_PRD(dt_data->pll_prd) | SET_PLL_FBD(dt_data->pll_fbd) |
		SET_PLL_FRS(dt_data->pll_frs);

	v4l2_dbg(1, debug, sd, "%s: Updating PLL clock of CSI TX%d, hsck=%d\n",
		 __func__, port - 1, get_hsck_freq(dt_data));
...
static void tc358840_set_ref_clk(struct v4l2_subdev *sd)
{
	...
	if (WARN_ON(dt_data->refclk_hz < 40000000 ||
		    dt_data->refclk_hz > 50000000))
		dt_data->refclk_hz = 42000000;

	/* System Frequency */
	sys_freq = dt_data->refclk_hz / 10000;
	i2c_wr8(sd, SYS_FREQ0, sys_freq & 0x00ff);
	i2c_wr8(sd, SYS_FREQ1, (sys_freq & 0xff00) >> 8);

	/* Audio System Frequency */
	lock_ref_freq = dt_data->refclk_hz / 100;
	...
	nco = (1649267442 / (dt_data->refclk_hz / 1000000));
	...
	nco = (1515264462 / (dt_data->refclk_hz / 1000000));
	...
	fh_min = dt_data->refclk_hz / 100000;
...
static void tc358840_set_csi(struct v4l2_subdev *sd)
{
	...
	for (port = CSI_TX_0; port <= CSI_TX_1; port++) {
		...
		if (dt_data->csi_port != CSI_TX_BOTH &&
		    dt_data->csi_port != port &&
		    !state->test_pattern) {
		...
		/* (0x0120) */
		i2c_wr32(sd, base_addr + LINEINITCNT, dt_data->lineinitcnt);

		/* (0x0254) */
		i2c_wr32(sd, base_addr + LPTXTIMECNT, dt_data->lptxtimecnt);
		/* (0x0258) */
		i2c_wr32(sd, base_addr + TCLK_HEADERCNT, dt_data->tclk_headercnt);
		/* (0x025C) */
		i2c_wr32(sd, base_addr + TCLK_TRAILCNT, dt_data->tclk_trailcnt);
		/* (0x0260) */
		i2c_wr32(sd, base_addr + THS_HEADERCNT, dt_data->ths_headercnt);
		/* (0x0264) */
		i2c_wr32(sd, base_addr + TWAKEUP, dt_data->twakeup);
		/* (0x0268) */
		i2c_wr32(sd, base_addr + TCLK_POSTCNT, dt_data->tclk_postcnt);
		/* (0x026C) */
		i2c_wr32(sd, base_addr + THS_TRAILCNT, dt_data->ths_trailcnt);
		/* (0x0270) */
		i2c_wr32(sd, base_addr + HSTXVREGCNT, dt_data->hstxvregcnt);
					    ...
					    struct v4l2_dv_timings *timings)
{
	...
	int target_fps;
	int clk = get_hsck_freq(&state->dt_data);
	int htot;
	int frame_height;
	int frame_width;
...
static void tc358840_initial_setup(struct v4l2_subdev *sd)
{
	...
	i2c_wr8_and_or(sd, DDC_CTL, ~(u8)(MASK_DDC_ACTION | MASK_DDC5V_MODE),
		       MASK_DDC_ACTION |
		       (dt_data->ddc5v_delay & MASK_DDC5V_MODE));
...
static void tc358840_delayed_work_poll(struct work_struct *work)
{
	...
	if (state->dt_data.enable_hdcp)
		v4l2_ctrl_s_ctrl(state->hdcp_repeater_ctrl,
				 state->dt_data.hdcp_repeater);
					   ...
					   bool *handled)
{
	...
	if ((hdcp_int & MASK_SHA_END) && state->dt_data.hdcp_repeater) {
		struct tc358840_state *state = to_state(sd);
	...
	if ((hdcp_int & MASK_AKSV_END) && state->dt_data.hdcp_repeater) {
		v4l2_dbg(1, debug, sd,
	...
	if ((hdcp_int & MASK_R0_END) && state->dt_data.hdcp_repeater) {
		struct tc358840_state *state = to_state(sd);
	...
	if ((hdcp_int & MASK_KM_END) && state->dt_data.hdcp_repeater) {
		v4l2_dbg(1, debug, sd,
			 "%s: HDCP MASK_KM_END\n", __func__);
...
static bool tc358840_parse_dt(struct tc358840_dt_data *dt_data,
			      struct i2c_client *client)
{
	...
	dt_data->reset_gpio = of_get_named_gpio(node, "reset-gpios", 0);
	if (dt_data->reset_gpio == 0)
		return false;
	...
	if (ret)
		goto put_node;
	dt_data->endpoint = endpoint;

	dt_data->enable_hdcp = of_property_read_bool(node, "enable_hdcp");
	v4l_dbg(1, debug, client, "enable_hdcp = %s\n",
		dt_data->enable_hdcp ? "true" : "false");

	dt_data->hdcp_repeater = false;
	if (dt_data->enable_hdcp) {
		dt_data->hdcp_repeater =
			of_property_read_bool(node, "hdcp_repeater");
		v4l_dbg(1, debug, client, "hdcp_repeater = %s\n",
			dt_data->hdcp_repeater ? "true" : "false");
	}

	if (of_property_read_u32(node, "ddc5v_delay", &dt_data->ddc5v_delay) ||
	    of_property_read_u32(node, "csi_port", &dt_data->csi_port) ||
	    of_property_read_u32(node, "lineinitcnt", &dt_data->lineinitcnt) ||
	    of_property_read_u32(node, "lptxtimecnt", &dt_data->lptxtimecnt) ||
	    of_property_read_u32(node, "tclk_headercnt", &dt_data->tclk_headercnt) ||
	    of_property_read_u32(node, "tclk_trailcnt", &dt_data->tclk_trailcnt) ||
	    of_property_read_u32(node, "ths_headercnt", &dt_data->ths_headercnt) ||
	    of_property_read_u32(node, "twakeup", &dt_data->twakeup) ||
	    of_property_read_u32(node, "tclk_postcnt", &dt_data->tclk_postcnt) ||
	    of_property_read_u32(node, "ths_trailcnt", &dt_data->ths_trailcnt) ||
	    of_property_read_u32(node, "hstxvregcnt", &dt_data->hstxvregcnt) ||
	    of_property_read_u32(node, "pll_prd", &dt_data->pll_prd) ||
	    of_property_read_u32(node, "pll_frs", &dt_data->pll_frs) ||
	    of_property_read_u32(node, "pll_fbd", &dt_data->pll_fbd))
		goto free_endpoint;

	if (dt_data->ddc5v_delay > DDC5V_DELAY_MAX)
		dt_data->ddc5v_delay = DDC5V_DELAY_MAX;
	v4l_dbg(1, debug, client, "ddc5v_delay = %u ms\n",
		50 * dt_data->ddc5v_delay);
	v4l_dbg(1, debug, client, "csi_port = %u\n", dt_data->csi_port);
	v4l_dbg(1, debug, client, "lineinitcnt = %u\n", dt_data->lineinitcnt);
	v4l_dbg(1, debug, client, "lptxtimecnt = %u\n", dt_data->lptxtimecnt);
	v4l_dbg(1, debug, client, "tclk_headercnt = %u\n", dt_data->tclk_headercnt);
	v4l_dbg(1, debug, client, "tclk_trailcnt = %u\n", dt_data->tclk_trailcnt);
	v4l_dbg(1, debug, client, "ths_headercnt = %u\n", dt_data->ths_headercnt);
	v4l_dbg(1, debug, client, "twakeup = %u\n", dt_data->twakeup);
	v4l_dbg(1, debug, client, "tclk_postcnt = %u\n", dt_data->tclk_postcnt);
	v4l_dbg(1, debug, client, "ths_trailcnt = %u\n", dt_data->ths_trailcnt);
...
static int tc358840_verify_chipid(struct v4l2_subdev *sd)
{
	...
	if ((cid & MASK_CHIPID) != TC358840_CHIPID) {
...
{
	int reset_gpio = state->dt_data.reset_gpio;
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (!tc358840_parse_dt(&state->dt_data, client)) {
		v4l_err(client, "Couldn't parse device tree\n");
		return -ENODEV;
	}

	state->refclk = devm_clk_get(&client->dev, "refclk");
	...
	state->dt_data.refclk_hz = clk_get_rate(state->refclk);
	...
	v4l2_dbg(1, debug, sd, "Releasing System Reset (gpio 0x%04X)\n",
		 state->dt_data.reset_gpio);
	if (!gpio_is_valid(state->dt_data.reset_gpio)) {
		v4l_err(client, "Reset GPIO is invalid!\n");
		err = state->dt_data.reset_gpio;
		goto err_clk_disable;
	}
	err = devm_gpio_request_one(&client->dev, state->dt_data.reset_gpio,
				    GPIOF_OUT_INIT_HIGH, "tc358840-reset");
	if (err) {
		dev_err(&client->dev,
			"Failed to request Reset GPIO 0x%04X: %d\n",
			state->dt_data.reset_gpio, err);
		goto err_clk_disable;
	}
