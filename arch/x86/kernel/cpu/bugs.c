...
spectre_v2_user_select_mitigation(enum spectre_v2_mitigation_cmd v2_cmd)
{
	...
	if (boot_cpu_has(X86_FEATURE_IBPB) && _ibpb_state != 0) {
...
static char *ibpb_state(void)
{
	if (boot_cpu_has(X86_FEATURE_IBPB)) {
		if (static_key_enabled(&switch_mm_always_ibpb) && x86_ibpb_enabled())
			return ", IBPB: always-on";
		if (static_key_enabled(&switch_mm_cond_ibpb) && x86_ibpb_enabled())
